"""Local candidate trend-hint acknowledgment status rollup — does NOT clear alerts or verify MRMS."""
...
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_trend_hint import (
    ...
    load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_trend_hint as load_candidate_trend_hint,
)
from backend.app.services.mrms_render_candidate_trend_hint_review_acknowledgment import (
    SUGGESTED_COMMAND as SUGGESTED_ACK_COMMAND,
    count_trend_hint_review_acknowledgments,
    load_latest_trend_hint_review_acknowledgment,
)
from backend.app.services.storage import LocalStorage

STATUS_JSON = "dev/mrms_render_candidate_trend_hint_ack_status.json"
STATUS_MD = "dev/mrms_render_candidate_trend_hint_ack_status.md"

SUGGESTED_COMMAND = "make mrms-render-candidate-trend-hint-ack-status"
...
def _classify_acknowledgment_status(
    *,
    ...
    if hint.get("trend") == TREND_NO_DATA or hint.get("hint_status") == HINT_MISSING:
        return ACK_STATUS_MISSING, False, "no_candidate_trend_hint"
...
def _suggested_action_for_status(
    *,
    ...
    if rollup_status == ROLLUP_BLOCKED:
        return "Resolve blocked candidate trend-hint safety gates before relying on acknowledgment status."
    if rollup_status == ROLLUP_MISSING:
        return "Refresh candidate trend hints before trend-hint acknowledgment status is available."
    if rollup_status == ROLLUP_NOT_NEEDED:
        return "Candidate trend stable — trend-hint review acknowledgment not required (local monitoring only)."
    if rollup_status == ROLLUP_NEEDS_ACKNOWLEDGMENT:
        return (
            "Record local trend-hint review acknowledgment after reviewing candidate trend hints "
            "(does not verify MRMS or clear alerts)."
        )
    if rollup_status == ROLLUP_STALE or stale_acknowledgment:
        return (
            "Re-review updated candidate trend hints and record a fresh trend-hint review acknowledgment "
            "(local advisory only)."
        )
    return "Trend-hint acknowledgment status current for latest candidate trend-hint snapshot."


def build_trend_hint_ack_status(storage: LocalStorage) -> dict[str, Any]:
    hint = load_candidate_trend_hint(storage) or build_candidate_trend_hint(storage)
    latest_ack = load_latest_trend_hint_review_acknowledgment(storage)
    acknowledgment_count = count_trend_hint_review_acknowledgments(storage)
...
def build_trend_hint_ack_status_markdown(status: dict[str, Any]) -> str:
    lines = [
        "# Candidate Trend-Hint Acknowledgment Status",
        "",
        f"Generated at: {status.get('generated_at')}",
        "",
        "> **WARNING:** Local trend-hint acknowledgment status rollup only. Advisory metadata — does **NOT** "
        ...
        "",
        "## Latest trend-hint review acknowledgment",
        "",
...
def load_trend_hint_ack_status(storage: LocalStorage) -> Optional[dict[str, Any]]:
...
def refresh_trend_hint_ack_status(storage: LocalStorage) -> dict[str, Any]:
    status = build_trend_hint_ack_status(storage)
    return save_trend_hint_ack_status(storage, status)


def compact_trend_hint_ack_status(storage: LocalStorage) -> dict[str, Any]:
    latest = load_trend_hint_ack_status(storage)
    if latest is None:
        latest = build_trend_hint_ack_status(storage)
    return {
        "available": load_trend_hint_ack_status(storage) is not None,
...
def build_trend_hint_ack_status_payload(storage: LocalStorage) -> dict[str, Any]:
    latest = load_trend_hint_ack_status(storage)
    if latest is None:
        latest = build_trend_hint_ack_status(storage)
    return {
