"""Local candidate trend-hint review acknowledgments — does NOT clear alerts or verify MRMS."""
...
ACKNOWLEDGMENTS_PATH = "dev/mrms_render_candidate_trend_hint_review_acknowledgments.json"
...
class TrendHintReviewAckValidationError(ValueError):
    """Raised when trend-hint review acknowledgment input fails validation."""
...
def load_trend_hint_review_acknowledgments(storage: LocalStorage) -> list[dict[str, Any]]:
...
def _latest_candidate_trend_hint(storage: LocalStorage) -> dict[str, Any]:
    return load_candidate_trend_hint(storage) or build_candidate_trend_hint(storage)


def validate_trend_hint_review_ack_input(
    *,
    ...
    if not identity:
        raise TrendHintReviewAckValidationError("operator_name or operator_initials is required")
    if not (note or "").strip():
        raise TrendHintReviewAckValidationError("note is required")


def create_trend_hint_review_acknowledgment(
    ...
    *,
    ...
    validate_trend_hint_review_ack_input(
        operator_name=operator_name,
        operator_initials=operator_initials,
        note=note,
    )

    hint = _latest_candidate_trend_hint(storage)
...
def load_latest_trend_hint_review_acknowledgment(storage: LocalStorage) -> Optional[dict[str, Any]]:
    entries = load_trend_hint_review_acknowledgments(storage)
    return entries[0] if entries else None


def count_trend_hint_review_acknowledgments(storage: LocalStorage) -> int:
    return len(load_trend_hint_review_acknowledgments(storage))


def compact_trend_hint_review_acknowledgment(entry: Optional[dict[str, Any]]) -> Optional[dict[str, Any]]:
    if entry is None:
        return None
    return {
...
def compact_trend_hint_review_acknowledgment_summary(storage: LocalStorage) -> dict[str, Any]:
    latest = load_latest_trend_hint_review_acknowledgment(storage)
    count = count_trend_hint_review_acknowledgments(storage)
    hint = _latest_candidate_trend_hint(storage)
    ...
    compact = compact_trend_hint_review_acknowledgment(latest) or {}
    return {
        "available": True,
        "count": count,
        "trend_review_still_recommended": trend_review_still_recommended,
...
def build_trend_hint_review_acknowledgments_payload(
    storage: LocalStorage,
    *,
    limit: int = 25,
) -> dict[str, Any]:
    ...
    hint = _latest_candidate_trend_hint(storage)
    latest = compact_trend_hint_review_acknowledgment(entries[0] if entries else None)
    return {
        "count": len(entries),
        ...
        "latest": latest,
        "entries": [
            compact_trend_hint_review_acknowledgment(item) for item in entries[:bounded] if item
        ],
        "suggested_command": SUGGESTED_COMMAND,
        "next_phase_recommendation": NEXT_PHASE_RECOMMENDATION,
        **_safety_fields(),
