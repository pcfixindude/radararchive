"""Gated sandbox acknowledgment history review — local advisory only; does NOT verify MRMS."""
...
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status_history import (
    SUGGESTED_COMMAND as SUGGESTED_ACK_HISTORY_COMMAND,
...
REVIEW_JSON = "dev/mrms_render_candidate_gated_ack_history_latest.json"
REVIEW_MD = "dev/mrms_render_candidate_gated_ack_history_latest.md"

SUGGESTED_COMMAND = "make mrms-review-gated-ack-history"
...
REVIEW_ACK_NOT_READY = "comparison_ack_not_ready"
REVIEW_ACK_BLOCKED = "comparison_ack_blocked"
REVIEW_ACK_HISTORY_MISSING = "ack_history_missing"
REVIEW_ACK_HISTORY_READY = "ack_history_ready"

NEXT_PHASE_TREND_HINT = (
    "Phase 100 — gated sandbox acknowledgment trend hint "
    "(local trend hint rollup without production authorization)"
)
NEXT_PHASE_PREFLIGHT = (
    "Phase 99 — resolve preflight evidence "
    "(until candidate_preflight_ready before acknowledgment history)"
)
NEXT_PHASE_DRY_RUN = (
    "Phase 99 — complete gated dry-run plan review "
    "(until dry_run_plan_ready before acknowledgment history)"
)
NEXT_PHASE_SCAFFOLD = (
    "Phase 99 — complete gated scaffold review "
    "(until scaffold_ready before acknowledgment history)"
)
NEXT_PHASE_LAYOUT = (
    "Phase 99 — complete gated sandbox layout review "
    "(until sandbox_layout_ready before acknowledgment history)"
)
NEXT_PHASE_MANIFEST_IO = (
    "Phase 99 — complete gated manifest import/export "
    "(until manifest_io ready before acknowledgment history)"
)
NEXT_PHASE_COMPARISON = (
    "Phase 99 — complete gated comparison history "
    "(until comparison_history_ready before acknowledgment history)"
)
NEXT_PHASE_TREND = (
    "Phase 99 — complete gated trend hint review "
    "(until trend_hint_ready before acknowledgment history)"
)
NEXT_PHASE_ACK = (
    "Phase 99 — complete gated comparison acknowledgment "
    "(until comparison_ack_ready before acknowledgment history)"
)
...
def _safety_fields() -> dict[str, Any]:
    return {
        "verified_mrms": False,
        "local_gated_ack_history_only": True,
        "advisory_only": True,
        ...
        "ack_history_ready_is_not_production_authorization": True,
...
def _comparison_ack_ready(ack_compact: dict[str, Any]) -> bool:
    rollup = ack_compact.get("rollup_status")
    if rollup == ROLLUP_NEEDS_ACKNOWLEDGMENT:
        return True
    if rollup in {ROLLUP_CURRENT, ROLLUP_NOT_NEEDED}:
        return True
    return False
    ...
    *,
    ...
    if preflight.get("preflight_level") == PREFLIGHT_NEEDS_REVIEW:
        for cmd in (
            ...
            f"{SUGGESTED_GATED_ACK_COMMAND} --refresh",
...
def _next_commands_ack_not_ready() -> list[str]:
    return [
        f"{SUGGESTED_GATED_ACK_COMMAND} --refresh",
        f"{SUGGESTED_GATED_TREND_COMMAND} --refresh",
        f"{SUGGESTED_COMMAND} --refresh",
...
def _next_commands_ack_history() -> list[str]:
    ...
    *,
    ...
    if dry_run_skipped or plan_compact.get("plan_status") != DRY_RUN_PLAN_READY:
        return (
            ...
            "Dry-run plan is not dry_run_plan_ready — acknowledgment history not run.",
    ...
    if scaffold_skipped or scaffold_compact.get("scaffold_status") != SCAFFOLD_READY:
        return (
            ...
            "Scaffold is not scaffold_ready — acknowledgment history not run.",
    ...
    if sandbox_skipped or not _sandbox_layout_ready(sandbox_compact):
        return (
            ...
            "Sandbox layout is not sandbox_layout_ready — acknowledgment history not run.",
    ...
    if manifest_io_skipped or not _manifest_io_ready(manifest_compact):
        return (
            ...
            "Manifest import/export is not ready — acknowledgment history not run.",
    ...
    if comparison_skipped or not _comparison_history_ready(comparison_compact):
        return (
            ...
            "Comparison history is not comparison_history_ready — acknowledgment history not run.",
    ...
    if ack_skipped or not _trend_hint_ready(trend_compact):
        ...
        if hint_status in {HINT_BLOCKED, HINT_MISSING}:
            return (
                ...
                "Trend hint blocked — acknowledgment history not run.",
                _next_commands_trend_not_ready(),
            )
        return (
            REVIEW_TREND_NOT_READY,
            "Trend hint is not trend_hint_ready — acknowledgment history not run.",
            _next_commands_trend_not_ready(),
        )

    if history_skipped or not _comparison_ack_ready(ack_compact):
        rollup_status = ack_compact.get("rollup_status")
        if rollup_status == ROLLUP_BLOCKED:
            return (
                REVIEW_ACK_BLOCKED,
                "Comparison acknowledgment blocked — acknowledgment history not run.",
                _next_commands_ack_not_ready(),
            )
        return (
            REVIEW_ACK_NOT_READY,
            "Comparison acknowledgment is not ready — acknowledgment history not run.",
            _next_commands_ack_not_ready(),
        )

    history_count = history_compact.get("history_count") or 0
    if history_count <= 0:
        return (
            REVIEW_ACK_HISTORY_MISSING,
            "Acknowledgment history missing — refresh acknowledgment status first.",
            _next_commands_ack_history(),
        )

    return (
        REVIEW_ACK_HISTORY_READY,
        "Acknowledgment history ready (local advisory) — consider trend hint review.",
        _next_commands_ack_history(),
    )


def _next_phase_for_review(review_status: str) -> str:
    if review_status == REVIEW_ACK_HISTORY_READY:
    ...
    if review_status in {REVIEW_ACK_NOT_READY, REVIEW_ACK_BLOCKED, REVIEW_ACK_HISTORY_MISSING}:
        return NEXT_PHASE_ACK
    return (
        "Phase 99 — resolve acknowledgment history or upstream gate blockers "
        "(depending on gated acknowledgment history report)"
    )


def review_gated_ack_history(storage: LocalStorage) -> dict[str, Any]:
    ...
    history_skipped = ack_skipped or not _comparison_ack_ready(ack_compact)

    if history_skipped:
        history_compact = compact_ack_status_history(storage)
        steps.append(
            _step_record(
                "ack_history",
                "(acknowledgment history skipped — comparison acknowledgment not ready)",
                {
                    "skipped": True,
                    "existing_history_count": history_compact.get("history_count"),
                    "existing_rollup_status": history_compact.get("latest_rollup_status"),
                },
            )
        )
    else:
        ...
        steps.append(
            _step_record(
                "ack_history",
                f"{SUGGESTED_ACK_HISTORY_COMMAND} --refresh",
                {
                    "skipped": False,
                    "history_count": history_compact.get("history_count"),
                    "latest_rollup_status": history_compact.get("latest_rollup_status"),
                    "latest_acknowledgment_status": history_compact.get("latest_acknowledgment_status"),
                    "latest_coverage_change": history_compact.get("latest_coverage_change"),
                },
            )
        )
    ...
    report = {
        ...
        "history_skipped": history_skipped,
        "ack_history_count": 0 if history_skipped else (history_compact.get("history_count") or 0),
        "latest_rollup_status": None if history_skipped else history_compact.get("latest_rollup_status"),
        "latest_acknowledgment_status": None
        if history_skipped
        else history_compact.get("latest_acknowledgment_status"),
        "latest_coverage_change": None if history_skipped else history_compact.get("latest_coverage_change"),
        "latest_recorded_at": None if history_skipped else history_compact.get("latest_recorded_at"),
        "recent_ack_history_entries": [] if history_skipped else (history_compact.get("recent_entries") or []),
    ...
    return save_gated_ack_history_report(storage, report)


def build_review_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Gated sandbox acknowledgment history review",
        "",
        "> **WARNING:** Local gated acknowledgment history review only. Advisory metadata — does **NOT** "
        ...
        f"- History skipped: {report.get('history_skipped')}",
        ...
        f"- Ack history count: {report.get('ack_history_count') if report.get('ack_history_count') is not None else '—'}",
        f"- Latest coverage change: {report.get('latest_coverage_change') or '—'}",
...
def save_gated_ack_history_report(storage: LocalStorage, report: dict[str, Any]) -> dict[str, Any]:
...
def load_gated_ack_history_report(storage: LocalStorage) -> Optional[dict[str, Any]]:
...
def compact_gated_ack_history(storage: LocalStorage) -> dict[str, Any]:
    latest = load_gated_ack_history_report(storage)
    if latest is None:
        ...
        return {
            ...
            "next_operator_step": "Run gated acknowledgment history after upstream gates open.",
            "json_path": _review_json_path(storage),
    ...
    return {
        ...
        "history_skipped": bool(latest.get("history_skipped")),
        "ack_history_count": latest.get("ack_history_count")
        if latest.get("ack_history_count") is not None
        else 0,
        "latest_rollup_status": latest.get("latest_rollup_status"),
        "latest_acknowledgment_status": latest.get("latest_acknowledgment_status"),
        "latest_coverage_change": latest.get("latest_coverage_change"),
        "latest_recorded_at": latest.get("latest_recorded_at"),
        "recent_ack_history_entries": latest.get("recent_ack_history_entries") or [],
...
def build_gated_ack_history_payload(storage: LocalStorage) -> dict[str, Any]:
    return {
        **_safety_fields(),
        "latest": load_gated_ack_history_report(storage),
