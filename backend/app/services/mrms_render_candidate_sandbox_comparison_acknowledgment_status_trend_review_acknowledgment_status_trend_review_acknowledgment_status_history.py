"""MRMS render candidate sandbox acknowledgment status trend review acknowledgment status trend review acknowledgment status history — local advisory only."""
...
ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY_JSON = (
    "dev/mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history.json"
)
ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY_MD = (
    "dev/mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history.md"
)

MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY = 25
SUGGESTED_COMMAND = (
    "make mrms-render-candidate-sandbox-comparison-acknowledgment-status-trend-review-acknowledgment-status-trend-review-acknowledgment-status-history"
)
...
def _history_json_path(storage: LocalStorage) -> str:
    return storage.normalize_path(ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY_JSON)


def _history_md_path(storage: LocalStorage) -> str:
    return storage.normalize_path(ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY_MD)


def load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
    storage: LocalStorage,
    *,
    limit: int = MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY,
) -> list[dict[str, Any]]:
    bounded = max(1, min(limit, MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY))
...
def _save_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
    storage: LocalStorage,
    ...
    storage.absolute_path(repo_path).write_text(
        json.dumps(
            entries[:MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY],
            indent=2,
            sort_keys=True,
        ),
        encoding="utf-8",
...
def build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_entry(
    status: dict[str, Any],
    *,
...
def append_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_entry(
    storage: LocalStorage,
    status: dict[str, Any],
) -> dict[str, Any]:
    history = load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
        storage,
        limit=MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY,
    )
    previous = history[0] if history else None
    entry = build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_entry(
        status,
...
def build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_markdown(
...
def refresh_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_report(
    storage: LocalStorage,
) -> dict[str, Any]:
    history = load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
        storage,
    ...
    storage.absolute_path(_history_md_path(storage)).write_text(
        build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_markdown(
            history, body
        ),
        encoding="utf-8",
    )
...
def compact_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
    storage: LocalStorage,
) -> dict[str, Any]:
    history = load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
        storage, limit=10
    ...
    return {
        ...
        "history_count": len(
            load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(storage)
        ),
        "latest_rollup_status": (latest_entry or {}).get("rollup_status"),
        "latest_acknowledgment_status": (latest_entry or {}).get("acknowledgment_status"),
        "latest_coverage_change": (latest_entry or {}).get("coverage_change"),
...
def build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_payload(
    storage: LocalStorage,
) -> dict[str, Any]:
    history = load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(storage)
    body = (
    ...
    return {
        **_safety_fields(),
        "latest": body,
        "compact": compact_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
            storage
        ),
        "entries": history,
        "count": len(history),
        "max_entries": MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY,
    }
