"""Gated sandbox comparison trend hint review — local advisory only; does NOT verify MRMS."""
...
REVIEW_JSON = "dev/mrms_render_candidate_gated_trend_review_latest.json"
REVIEW_MD = "dev/mrms_render_candidate_gated_trend_review_latest.md"

SUGGESTED_COMMAND = "make mrms-review-gated-trend"
...
REVIEW_TREND_NEEDS_REVIEW = "trend_hint_needs_review"
REVIEW_TREND_READY = "trend_hint_ready"

NEXT_PHASE_ACK = (
    "Phase 98 — gated sandbox comparison acknowledgment "
    "(local acknowledgment rollup without production authorization)"
)
NEXT_PHASE_PREFLIGHT = (
    "Phase 97 — resolve preflight evidence "
    "(until candidate_preflight_ready before trend hint review)"
)
NEXT_PHASE_DRY_RUN = (
    "Phase 97 — complete gated dry-run plan review "
    "(until dry_run_plan_ready before trend hint review)"
)
NEXT_PHASE_SCAFFOLD = (
    "Phase 97 — complete gated scaffold review "
    "(until scaffold_ready before trend hint review)"
)
NEXT_PHASE_LAYOUT = (
    "Phase 97 — complete gated sandbox layout review "
    "(until sandbox_layout_ready before trend hint review)"
)
NEXT_PHASE_MANIFEST_IO = (
    "Phase 97 — complete gated manifest import/export "
    "(until manifest_io ready before trend hint review)"
)
NEXT_PHASE_COMPARISON = (
    "Phase 97 — complete gated comparison history "
    "(until comparison_history_ready before trend hint review)"
)
...
def _safety_fields() -> dict[str, Any]:
    return {
        "verified_mrms": False,
        "local_gated_trend_review_only": True,
        "advisory_only": True,
...
def _next_commands_trend_hint() -> list[str]:
    ...
    *,
    ...
    if preflight.get("preflight_level") != PREFLIGHT_CANDIDATE_READY:
        return (
            ...
            "Preflight is not candidate_preflight_ready — trend hint not run.",
    ...
    if dry_run_skipped or plan_compact.get("plan_status") != DRY_RUN_PLAN_READY:
        return (
            ...
            "Dry-run plan is not dry_run_plan_ready — trend hint not run.",
    ...
    if scaffold_skipped or scaffold_compact.get("scaffold_status") != SCAFFOLD_READY:
        return (
            ...
            "Scaffold is not scaffold_ready — trend hint not run.",
    ...
    if sandbox_skipped or not _sandbox_layout_ready(sandbox_compact):
        return (
            ...
            "Sandbox layout is not sandbox_layout_ready — trend hint not run.",
    ...
    if manifest_io_skipped or not _manifest_io_ready(manifest_compact):
        return (
            ...
            "Manifest import/export is not ready — trend hint not run.",
            _next_commands_manifest_io_not_ready(),
        )

    if trend_skipped or not _comparison_history_ready(comparison_compact):
        return (
            REVIEW_COMPARISON_NOT_READY,
            "Comparison history is not comparison_history_ready — trend hint not run.",
    ...
    if hint_status in {HINT_BLOCKED, HINT_MISSING}:
        return (
            ...
            "Trend hint blocked — resolve blockers before advancing.",
            _next_commands_trend_hint(),
        )
    if hint_status == HINT_NEEDS_REVIEW:
        return (
            REVIEW_TREND_NEEDS_REVIEW,
            "Trend hint needs review (local advisory) — inspect recurring comparison signals.",
            _next_commands_trend_hint(),
        )
    if hint_status == HINT_READY:
        return (
            REVIEW_TREND_READY,
            "Trend hint ready (local advisory) — consider acknowledgment review.",
            _next_commands_trend_hint(),
        )

    return (
        REVIEW_TREND_BLOCKED,
        "Trend hint blocked — resolve blocking items before advancing.",
        _next_commands_trend_hint(),
    )


def _next_phase_for_review(review_status: str) -> str:
    if review_status in {REVIEW_TREND_READY, REVIEW_TREND_NEEDS_REVIEW}:
    ...
    return (
        "Phase 97 — resolve trend hint or upstream gate blockers "
        "(depending on gated trend review report)"
    )


def review_gated_trend_review(storage: LocalStorage) -> dict[str, Any]:
    ...
    if trend_skipped:
        ...
        steps.append(
            _step_record(
                "trend_hint",
                f"{SUGGESTED_TREND_HINT_COMMAND} --refresh",
                {
                    "skipped": False,
                    "hint_status": trend_compact.get("hint_status"),
    ...
    report = {
        ...
        "suggested_action": None if trend_skipped else trend_compact.get("suggested_action"),
        "resolution_status": blockers_compact.get("resolution_status"),
    ...
    return save_gated_trend_review_report(storage, report)


def build_review_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# Gated sandbox comparison trend hint review",
        "",
        "> **WARNING:** Local gated trend hint review only. Advisory metadata — does **NOT** "
...
def save_gated_trend_review_report(storage: LocalStorage, report: dict[str, Any]) -> dict[str, Any]:
...
def load_gated_trend_review_report(storage: LocalStorage) -> Optional[dict[str, Any]]:
...
def compact_gated_trend_review(storage: LocalStorage) -> dict[str, Any]:
    latest = load_gated_trend_review_report(storage)
    if latest is None:
        ...
        return {
            ...
            "next_operator_step": "Run gated trend hint review after upstream gates open.",
            "json_path": _review_json_path(storage),
...
def build_gated_trend_review_payload(storage: LocalStorage) -> dict[str, Any]:
    return {
        **_safety_fields(),
        "latest": load_gated_trend_review_report(storage),
