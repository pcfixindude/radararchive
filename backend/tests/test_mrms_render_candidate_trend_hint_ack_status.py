"""Tests for candidate trend-hint acknowledgment status rollup (Phase 82)."""
...
from backend.app.services.mrms_render_candidate_trend_hint_ack_status import (
    ...
    build_trend_hint_ack_status_markdown,
...
def test_status_json_and_markdown_persistence(storage, monkeypatch):
    ...
    assert "trend-hint acknowledgment status" in markdown.lower()
    assert "Rollup status" in build_trend_hint_ack_status_markdown(status)
...
def test_summary_includes_status_compact(db_session, storage, monkeypatch):
    ...
    compact = summary["mrms_render_candidate_trend_hint_ack_status"]
...
def test_status_get_endpoint(client, storage, monkeypatch):
    _seed_candidate_trend_hint_needs_review(storage, monkeypatch)
    response = client.get("/api/validation/mrms-render-candidate/sandbox/trend-hint-ack-status")
...
def test_status_post_endpoint(client, storage, monkeypatch):
    _seed_candidate_trend_hint_needs_review(storage, monkeypatch)
    response = client.post("/api/validation/mrms-render-candidate/sandbox/trend-hint-ack-status")
    assert response.status_code == 200
    assert storage.absolute_path(STATUS_JSON).is_file()
