"""Tests for trend review acknowledgment status trend review acknowledgment status history (Phase 79)."""
...
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history import (
    ...
    COVERAGE_UNCHANGED,
    COVERAGE_WORSENED,
    MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY,
    append_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_entry,
    build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_entry,
    compact_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history,
...
def test_history_bounded_to_max_entries(storage, monkeypatch):
    _use_test_storage(monkeypatch, storage)
    for index in range(MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY + 5):
        status = {
    ...
    history = load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(
        storage,
        limit=MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY + 10,
    )
    assert len(history) == MAX_ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY
...
def test_history_json_and_markdown_persistence(storage, monkeypatch):
    ...
    markdown = storage.absolute_path(
        ACK_STATUS_TREND_REVIEW_ACK_STATUS_TREND_REVIEW_ACK_STATUS_HISTORY_MD
...
def test_history_safety_invariants(storage, monkeypatch):
    ...
    entry = load_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history(storage)[0]
...
def test_summary_includes_history_compact(db_session, storage, monkeypatch):
    ...
    compact = summary[
        "mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history"
    ]
...
def test_history_get_endpoint(client, storage, monkeypatch):
    ...
    response = client.get(
        ...
        "comparison-acknowledgment-status/trend-review-acknowledgment-status/trend-review-acknowledgment-status/history"
    )
...
def test_build_entry_includes_change_fields(storage, monkeypatch):
    _use_test_storage(monkeypatch, storage)
    previous = build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_entry(
        {"rollup_status": ROLLUP_NEEDS_ACKNOWLEDGMENT, "acknowledgment_status": "none"},
        previous_entry=None,
    )
    latest = build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_history_entry(
        {
