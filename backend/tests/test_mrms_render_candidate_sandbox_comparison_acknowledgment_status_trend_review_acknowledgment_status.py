"""Tests for acknowledgment status trend review acknowledgment status rollup (Phase 74)."""
...
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment_status import (
    ...
    ROLLUP_STALE,
    STATUS_JSON,
    build_ack_status_trend_review_acknowledgment_status,
    build_ack_status_trend_review_acknowledgment_status_markdown,
...
def test_status_json_and_markdown_persistence(storage, monkeypatch):
    _seed_status_trend_hint_needs_review(storage, monkeypatch)
    status = refresh_ack_status_trend_review_acknowledgment_status(storage)
    ...
    assert "Rollup status" in build_ack_status_trend_review_acknowledgment_status_markdown(status)
...
def test_status_get_endpoint(client, storage, monkeypatch):
    ...
    response = client.get(
        ...
        "comparison-acknowledgment-status/trend-review-acknowledgment-status"
    )
