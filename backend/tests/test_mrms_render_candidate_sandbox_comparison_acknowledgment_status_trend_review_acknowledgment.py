"""Tests for acknowledgment status trend hint review acknowledgments (Phase 73)."""
...
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment import (
    ...
    load_ack_status_trend_review_acknowledgments,
)
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status import (
...
def test_acknowledgment_requires_operator_and_note(storage, monkeypatch):
    _use_test_storage(monkeypatch, storage)
    try:
        create_ack_status_trend_review_acknowledgment(storage, note="reviewed")
...
def test_acknowledgment_safety_invariants(storage, monkeypatch):
    ...
    compact = compact_ack_status_trend_review_acknowledgment_summary(storage)
...
def test_summary_includes_acknowledgment_compact(db_session, storage, monkeypatch):
    ...
    compact = summary["mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment"]
...
def test_acknowledgment_get_endpoint(client, storage, monkeypatch):
    ...
    response = client.get(
        "/api/validation/mrms-render-candidate/sandbox/import-export/comparison-acknowledgment-status/trend-review-acknowledgments"
    )
...
def test_acknowledgments_payload_lists_entries(storage, monkeypatch):
    _seed_status_trend_hint_needs_review(storage, monkeypatch)
    create_ack_status_trend_review_acknowledgment(storage, operator_initials="A", note="First.")
    create_ack_status_trend_review_acknowledgment(storage, operator_initials="B", note="Second.")
    payload = build_ack_status_trend_review_acknowledgments_payload(storage, limit=10)
