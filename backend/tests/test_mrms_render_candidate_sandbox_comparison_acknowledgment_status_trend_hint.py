"""Tests for MRMS render candidate sandbox acknowledgment status trend hints (Phase 72)."""
...
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_hint import (
    ...
    build_ack_status_trend_hint_markdown,
...
def _seed_status_history(storage, monkeypatch, *, count: int = 2):
...
def _seed_history_entries(storage, entries: list[dict]):
    _save_ack_status_history(storage, entries)
...
def test_trend_hint_from_status_refresh_workflow(storage, monkeypatch):
    _seed_status_history(storage, monkeypatch, count=1)
    hint = build_ack_status_trend_hint(storage)
...
def test_trend_hint_json_and_markdown_persistence(storage, monkeypatch):
    _seed_status_history(storage, monkeypatch, count=1)
    hint = refresh_ack_status_trend_hint(storage)
    assert storage.absolute_path(HINT_JSON).is_file()
    markdown = storage.absolute_path(hint["markdown_path"]).read_text(encoding="utf-8")
    assert "status trend hints only" in markdown.lower()
    assert "Trend summary" in build_ack_status_trend_hint_markdown(hint)
...
def test_summary_includes_trend_hint_compact(db_session, storage, monkeypatch):
    ...
    compact = summary["mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_hint"]
...
def test_trend_hint_get_endpoint(client, storage, monkeypatch):
    _seed_status_history(storage, monkeypatch, count=1)
    response = client.get(
        "/api/validation/mrms-render-candidate/sandbox/import-export/comparison-acknowledgment-status/trend-hint"
    )
