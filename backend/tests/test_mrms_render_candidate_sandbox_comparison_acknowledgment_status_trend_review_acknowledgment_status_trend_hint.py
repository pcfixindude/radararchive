"""Tests for acknowledgment status trend review acknowledgment status trend hints (Phase 76)."""
...
from backend.app.services.mrms_render_candidate_sandbox_comparison_acknowledgment_status_trend_review_acknowledgment_status_trend_hint import (
    ...
    build_ack_status_trend_review_acknowledgment_status_trend_hint_markdown,
...
def _seed_status_trend_review_ack_status_history(storage, monkeypatch, *, count: int = 1):
...
def test_trend_hint_from_status_refresh_workflow(storage, monkeypatch):
    _seed_status_trend_review_ack_status_history(storage, monkeypatch, count=1)
    hint = build_ack_status_trend_review_acknowledgment_status_trend_hint(storage)
...
def test_trend_hint_json_and_markdown_persistence(storage, monkeypatch):
    ...
    assert "trend review acknowledgment status trend hints only" in markdown.lower()
    assert "Trend summary" in build_ack_status_trend_review_acknowledgment_status_trend_hint_markdown(hint)
...
def test_trend_hint_get_endpoint(client, storage, monkeypatch):
    ...
    response = client.get(
        ...
        "comparison-acknowledgment-status/trend-review-acknowledgment-status/trend-hint"
    )
