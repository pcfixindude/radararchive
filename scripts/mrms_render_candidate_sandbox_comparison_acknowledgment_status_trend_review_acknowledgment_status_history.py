"""Refresh local sandbox comparison acknowledgment status trend review acknowledgment status history (Phase 75)."""
...
def main() -> None:
    ...
    if args.refresh:
        report = refresh_ack_status_trend_review_acknowledgment_status_history_report(storage)
    else:
        report = build_ack_status_trend_review_acknowledgment_status_history_payload(storage)["latest"]
