"""Refresh local sandbox comparison acknowledgment status trend hints (Phase 72)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh local sandbox comparison acknowledgment status trend hints (Phase 72)."
    )
    ...
    print(
        "WARNING: Acknowledgment status trend hints are local advisory only — "
    ...
    if args.refresh:
        ...
        hint = build_ack_status_trend_hint_payload(storage)["latest"]
