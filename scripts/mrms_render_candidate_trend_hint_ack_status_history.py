"""Refresh local candidate trend-hint acknowledgment status history (Phase 83)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Refresh local candidate trend-hint acknowledgment status history (Phase 83)."
    )
    ...
    print(
        "WARNING: Trend-hint acknowledgment status history is local advisory only — "
    ...
    if args.refresh:
        report = refresh_trend_hint_ack_status_history_report(storage)
    else:
        report = build_trend_hint_ack_status_history_payload(storage)["latest"]
