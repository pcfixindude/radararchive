"""Refresh local sandbox comparison acknowledgment status trend review acknowledgment status trend review acknowledgment status rollup (Phase 78)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "trend review acknowledgment status rollup (Phase 78)."
        )
    )
    ...
    print(
        "WARNING: Trend review acknowledgment status rollup is local advisory only — "
    ...
    if args.refresh:
        ...
        status = build_ack_status_trend_review_acknowledgment_status_trend_review_acknowledgment_status_payload(
            storage
        )["latest"]
