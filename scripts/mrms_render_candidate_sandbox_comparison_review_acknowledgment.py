"""Record local sandbox comparison trend hint review acknowledgment (Phase 69 — does NOT verify MRMS)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Record local sandbox comparison trend hint review acknowledgment (Phase 69)."
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Mark that operator reviewed the current trend recommendation",
    )
    ...
    print(
        "WARNING: Sandbox comparison review acknowledgment is local only — "
    ...
    if args.list:
        payload = build_sandbox_comparison_review_acknowledgments_payload(storage)
        if args.json:
    ...
    print("Sandbox comparison review acknowledgment recorded (local only — does NOT clear alerts):")
