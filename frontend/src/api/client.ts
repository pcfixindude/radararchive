...
export type MrmsRenderCandidateGatedScaffoldReviewCompact = {
  available?: boolean;
  review_status?: string | null;
  preflight_level?: string | null;
  preflight_reason?: string | null;
  ...
  next_phase_recommendation?: string | null;
  verified_mrms: boolean;
  local_gated_scaffold_review_only: boolean;
  advisory_only: boolean;
  does_not_clear_alerts: boolean;
  does_not_enable_production: boolean;
  does_not_download_or_decode: boolean;
...
export type MrmsRenderCandidateGatedComparisonHistoryCompact = {
  available?: boolean;
  review_status?: string | null;
  preflight_level?: string | null;
  preflight_reason?: string | null;
  ...
  import_export_reason?: string | null;
  manifest_io_blockers?: string[];
  comparison_skipped?: boolean;
  history_status?: string | null;
  history_reason?: string | null;
  history_count?: number;
  comparison_history_blockers?: string[];
  comparison_history_warnings?: string[];
  ...
  next_phase_recommendation?: string | null;
  verified_mrms: boolean;
  local_gated_comparison_history_only: boolean;
  advisory_only: boolean;
  does_not_clear_alerts: boolean;
  does_not_enable_production: boolean;
  does_not_download_or_decode: boolean;
...
export type MrmsRenderCandidateGatedTrendReviewCompact = {
  available?: boolean;
  review_status?: string | null;
  preflight_level?: string | null;
  preflight_reason?: string | null;
  ...
  trend_blockers?: string[];
  trend_warnings?: string[];
  ...
  next_phase_recommendation?: string | null;
  verified_mrms: boolean;
  local_gated_trend_review_only: boolean;
  advisory_only: boolean;
  does_not_clear_alerts: boolean;
  does_not_enable_production: boolean;
  does_not_download_or_decode: boolean;
...
export type MrmsRenderCandidateGatedComparisonAckCompact = {
  available?: boolean;
  review_status?: string | null;
  preflight_level?: string | null;
  preflight_reason?: string | null;
  ...
  next_phase_recommendation?: string | null;
  verified_mrms: boolean;
  local_gated_comparison_ack_only: boolean;
  advisory_only: boolean;
  does_not_clear_alerts: boolean;
  does_not_enable_production: boolean;
  does_not_download_or_decode: boolean;
...
export async function refreshRenderCandidateGatedScaffoldReview(): Promise<
  | { ok: true; data: { compact: MrmsRenderCandidateGatedScaffoldReviewCompact } }
  | { ok: false; error: string }
> {
  try {
    const response = await fetch(
      `${API_BASE}/api/validation/mrms-render-candidate/sandbox/gated-scaffold-review`,
      { method: 'POST' },
    );
    if (!response.ok) {
      return {
        ok: false,
        error: `Gated scaffold review failed (${response.status})`,
      };
    }
...
export async function refreshRenderCandidateGatedComparisonHistory(): Promise<
  | { ok: true; data: { compact: MrmsRenderCandidateGatedComparisonHistoryCompact } }
  | { ok: false; error: string }
> {
  try {
    const response = await fetch(
      `${API_BASE}/api/validation/mrms-render-candidate/sandbox/gated-comparison-review`,
      { method: 'POST' },
    );
    if (!response.ok) {
      return {
        ok: false,
        error: `Gated comparison history review failed (${response.status})`,
      };
    }
    const data = (await response.json()) as { compact: MrmsRenderCandidateGatedComparisonHistoryCompact };
    return { ok: true, data };
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : 'Unknown error' };
  }
...
export async function refreshRenderCandidateGatedTrendReview(): Promise<
  | { ok: true; data: { compact: MrmsRenderCandidateGatedTrendReviewCompact } }
  | { ok: false; error: string }
> {
  try {
    const response = await fetch(
      `${API_BASE}/api/validation/mrms-render-candidate/sandbox/gated-trend-review`,
      { method: 'POST' },
    );
    if (!response.ok) {
      return {
        ok: false,
        error: `Gated trend hint review failed (${response.status})`,
      };
    }
    const data = (await response.json()) as { compact: MrmsRenderCandidateGatedTrendReviewCompact };
    return { ok: true, data };
  } catch (error) {
    return { ok: false, error: error instanceof Error ? error.message : 'Unknown error' };
  }
...
export async function refreshRenderCandidateGatedComparisonAck(): Promise<
  | { ok: true; data: { compact: MrmsRenderCandidateGatedComparisonAckCompact } }
  | { ok: false; error: string }
> {
  try {
    const response = await fetch(
      `${API_BASE}/api/validation/mrms-render-candidate/sandbox/gated-ack-review`,
      { method: 'POST' },
    );
    if (!response.ok) {
      return {
        ok: false,
        error: `Gated comparison acknowledgment review failed (${response.status})`,
      };
    }
