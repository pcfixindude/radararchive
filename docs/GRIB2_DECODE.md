...
`make mrms-review-session` records a local MRMS proof review session linking escalation, digest, handoff, diff, and bundle evidence. `make mrms-review-sessions` lists bounded session history. `make mrms-review-session-compare` compares the latest session against the previous one and persists local comparison metadata. Review sessions and comparison do not verify MRMS or enable production rendering.
