...
`make mrms-review-session` records a local MRMS proof review session linking escalation, digest, handoff, diff, and bundle evidence. `make mrms-review-sessions` lists bounded session history. `make mrms-review-session-compare` compares the latest session against the previous one. `make mrms-review-session-export` writes a local Markdown export with comparison, guidance, and digest hints. `make mrms-review-session-export-diff` compares consecutive exports; `make mrms-review-session-export-diff-history` lists bounded diff history. `make mrms-review-session-export-diff-trend` summarizes export diff trends (improving/worsening/mixed/stable). `make mrms-review-session-export-diff-trend-hint` recommends when to create a new review session or re-export. `make operator-review-status` consolidates local review hints into one status block (`status_level`, recommendations, `top_suggested_command`, `evidence_trend`, visual review regeneration fields, runbook `guidance_items`). `make operator-workflow-presets` lists grouped local workflow presets with recommended flags (including `regenerate-visual-review` when visual review is stale and `full-scheduled-proof-review-with-visual-review` for scheduled opt-in), runbook guidance, short reasons, and copy-ready commands derived from operator review status. `make scheduled-proof-bundle-visual-review` runs full scheduled proof review with explicit visual review generation (Phase 59). `make mrms-visual-review` inspects existing local tile/render artifacts and writes JSON + Markdown visual review manifests (does not decode or download). `make mrms-visual-review-compare` and `make mrms-visual-review-hint` compare manifests and suggest regeneration when proof/validation evidence is newer. Dev Validation summary includes `operator_review_status`, `operator_workflow_presets`, `mrms_visual_review`, `mrms_visual_review_comparison`, `mrms_visual_review_hint`, `scheduled_operator_status`, and recent export diff history (`mrms_review_session_export_diff_history`, max 5 entries). Scheduled validation with `--review-export` reports `review_export_trend_hint` after the review export step. Optional `--export-after-create` on `make mrms-review-session` exports Markdown immediately after session create (session kept if export fails). `make scheduled-proof-bundle-review-export` runs proof → bundle → diff → handoff → digest → review export in one local sequence (skips export safely when no review session exists). Review sessions, comparison, export, export diff, auto-export, consolidated status, and scheduled export do not verify MRMS or enable production rendering.
